...
async function testAgentFinalInjectsCacheableOmbrePolicyForBtombreMcp() {
    ...
    try {
        ...
        assert.equal(res.status, 200);
        assert.equal(finalRequests.length, 1);
        assert.deepEqual(finalRequests[0].body.cache_control, { type: 'ephemeral' });
        ...
        assert.equal(chatEvent.final_mcp.automatic_cache_control, '5m');
    } finally {
        globalThis.fetch = originalFetch;
    }
}

async function testAgentFinalCachesStableNuojijiSystemPrefixForOneHour() {
    const app = createApp();
    const env = {
        OUTBOX: new FakeKv(),
        RELAY_SECRET: 'test-secret',
    ...
    const originalFetch = globalThis.fetch;
    const finalRequests = [];
    const stablePrompt = 'Stable character instruction. Keep this reusable. '.repeat(80);
    ...
    try {
        ...
        assert.match(system[1].text, /Stable character instruction/);
        assert.doesNotMatch(system[1].text, /Recent:/);
        assert.match(system[2].text, /^Recent:\nUser: dynamic hello/);
...
async function testAgentFinalSortsMcpToolsByNameForCacheStability() {
    const app = createApp();
    const env = {
        OUTBOX: new FakeKv(),
        RELAY_SECRET: 'test-secret',
        ...
        if (textUrl.includes('brain.btombre.men')) {
            ...
            if (body.method === 'tools/list') {
                return new Response(JSON.stringify({
                    ...
                    result: {
                        tools: [
                            { name: 'trace', description: 'Modify memory.', inputSchema: { type: 'object', properties: {} } },
                            { name: 'breath', description: 'Read memory.', inputSchema: { type: 'object', properties: {} } },
                            { name: 'hold', description: 'Write memory.', inputSchema: { type: 'object', properties: {} } },
                        ],
                    },
                }), {
                    status: 200,
    ...
    try {
        ...
        assert.equal(res.status, 200);
        assert.deepEqual(finalRequests[0].body.tools.map((tool) => tool.name), ['breath', 'hold', 'trace']);
    } finally {
        globalThis.fetch = originalFetch;
    }
}
...
await testAgentFinalSortsMcpToolsByNameForCacheStability();
